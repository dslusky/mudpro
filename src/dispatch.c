    ...
    parse_regexp_t *parse_regexp, gint *ovector, gint strings)
{
    ...
    else if (!strcasecmp (action->type, "PrintStats"))
    {
        GString *buf;
        stats_exp_update ();
        buf = commified_g_string ((int)stats.exp.gained);
        prints ("(%s total) (%s) (eta: %s)", buf->str, stats.exp.rate_str->str, stats.exp.eta_str->str);
